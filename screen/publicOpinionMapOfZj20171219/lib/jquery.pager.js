...
(function($, window, document, undefined) {
	...
	Pager.prototype = {
		...
		renderHtml: function() {
			...
			options.pageCount = Math.ceil(options.itemCount / options.pageSize)-1;
			var html = [];

			//生成上一页的按钮
			if(options.pageIndex > 0) {
				html.push('<a page="' + (options.pageIndex - 1) + '" ' + ' class="flip">' + options.prevText + '</a>');
			} else {
			...
			for(var i = startIndex; i <= endIndex; i++) {
				if(options.pageIndex == i) {
					if(i == endIndex) {
						...
						html.push('<span class="curPage">' + (i + 1) + '</span><span class="m-line">／</span>');
					}
					
				} else {
					if(i == endIndex) {
						html.push('<a page="' + i + '"' + '>' + (i + 1) + '</a>');
					} else {
						html.push('<a page="' + i + '"' + '>' + (i + 1) + '</a><span class="m-line">／</span>');
					}
			...
			if(endIndex < options.pageCount - 1) {
				html.push("<span>...</span> ");
				html.push("<a " + " page='" + (options.pageCount - 1) + "'>" + options.pageCount + "</a> ");
			}

			//生成下一页的按钮
			if(options.pageIndex < options.pageCount - 1) {
				html.push('<a page="' + (options.pageIndex + 1) + '" ' + ' class="flip">' + options.nextText + '</a>');
			} else {
		...
		bindEvent: function() {
			var that = this;
			that.$ele.unbind("click").on("click", "a", function() {
