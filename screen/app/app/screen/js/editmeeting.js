    ...
    EditApp.prototype.focusnextlist = function () {
        ...
        if(self.listdata){
            ...
            $('.listchoose').remove();
            $('.reportlist').animate({scrollTop:(viewportTOP-firstviewportTOP)},"500")
    ...
    EditApp.prototype.rendermetalog = function () {
        ...
        var metastatus = [];
		console.log(metalogdata);
        if(metalogdata == 0){
            $('.metalog-start').attr('src','img/notstart_03.png');
              $('.metalog-process:lt(4)').attr('src','img/metalog-notpro.png')
        }
        else {
		    ...
		    if(metastatus.indexOf('送审') !=-1||metastatus.indexOf('待审') !=-1){
		    		$('.metalog-start').attr('src','img/metalog-start.png');
            ...
            if(metastatus.indexOf("签发") != -1){
            	 ...
            	 $('.metalog-process:eq(2)').attr('src','img/metalog-notpro.png');
              	...
              	if( 
              		metastatus.indexOf("撤稿") != -1 || metastatus.indexOf("传稿") != -1||metastatus.indexOf("提交") != -1){
            		 $('.metalog-start').attr('src','img/metalog-start.png')
            ...
            if(detalisH + adddataH > 672){
                ...
                $('.adddata-clickdown').on('click', function () {
                    ...
                    if( scrollH >(data.length-2)*140 ){
                        $('.center-wrap').animate({scrollTop:0},"600")
                        return false
                    }
                    $('.center-wrap').animate({scrollTop:scrollH+140},"600")
    ...
    EditApp.prototype.renderright = function () {
        ...
        if($.isEmptyObject(taskdatalog) == false){
            ...
            if(taskdatalog.Materials.length == 0){
                ...
                $('.metalogpro').css({
                    'height':'424px','padding':'47px 8px','margin-top':'33px'
                });
                $('.metalog-box').css('height','548px');
                $('.metalog-start').css({
                    'width':'456px','height':'43px'
                });
                $('.metalog-img').css({
                    'width':'456px','height':'55px'
                });
...
//              switch (taskdatalog.PlanTaskLogs[0].OPERSTATUS){
//                  case "10" :
//                       $('.tasklogpro:eq(0)').addClass('tasklogdone');
//                      $('.tasklogpro :eq(1)').addClass('tasklognotdone');
//                      $('.tasklogpro :eq(2)').addClass('tasklognotdone');
//                      $('.tasklogpro :eq(3)').addClass('tasklognotdone');
//                      
//                      break;
//                  case "1":
//                      $('.tasklogpro :eq(1)').addClass('tasklogdone');
//                      $('.tasklogpro :eq(0)').addClass('tasklognotdone');
//                      $('.tasklogpro :eq(2)').addClass('tasklognotdone');
//                      $('.tasklogpro :eq(3)').addClass('tasklognotdone');
//                      break;
//                  case "11":
...
//                      $('.tasklogpro :eq(1)').addClass('tasklognotdone');
//                      $('.tasklogpro :eq(2)').addClass('tasklognotdone');
//                      $('.tasklogpro :eq(3)').addClass('tasklogdone');
//                      break
//              }
            }
            else {
    ...
    EditApp.prototype.rendermaterials = function () {
        ...
        $('.mater-img').on('click', function () {
            ...
            d3.select('#bigbox').append('img').classed('big-img',true).attr({"src":src,"id":"bigimg"})
            $('#bigbox').show().animate({"width":"602px","height":"325px","left":"0","top":"0"},600);
            //放大图片点击缩小
            $('#bigimg').on('click', function () {
                $(this).remove();
                $('#bigbox').animate({"width":"0","height":"0","left":"301px","top":"162px"},600, function () {
                    $('#bigbox').hide()
    ...
    EditApp.prototype.materroll = function () {
        ...
        if(leftscroll+602 >= divwidth){
            $('.taskMaterials').animate({scrollLeft:0},"2000");
            self.rollindex = 0
        }
        else {

            $('.taskMaterials').animate({scrollLeft:self.rollindex*191},"2000", function () {
                self.rollindex++;
            });
        }
    }
    ...
    EditApp.prototype.clickvideo = function (src,id) {
        ...
        $("#bigbox").append($video).animate({"width":"602px","height":"325px","left":"0","top":"0"},600)
        videojs(id,{
            ControlBar: {
    ...
    EditApp.prototype.getmediaurl = function (id) {
        ...
        var masurl = "/mlfvideo/openapi/pages.do?method=prePlay&appKey=TRSWCM8"
        var videourl;
        $.ajax({
            type:"get",
            headers:{
