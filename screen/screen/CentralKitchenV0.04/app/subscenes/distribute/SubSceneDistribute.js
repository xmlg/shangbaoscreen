define(function(require) {
    ...
    function SubSceneDistribute(parent) {
        ...
        self.zjxwAppP = dataManager.getData().zjxwAppPub;
        self.mainLabel = new MainLabel(self, '发', self.value + self.zjxwAppP);
        ...
        this.threeDistribute = new ThreeDistribute(self, 235, 295);
        this.threeDistribute.setPosition(900, 915);
    ...
    SubSceneDistribute.prototype.update = function() {
        ...
        newValue += zjxwAppP;
        var diffValue = newValue - (value + self.zjxwAppP);
            ...
            util.sliceNumber(diffValue, 5).forEach(function(d, i) {
                delay(5000 * Math.random(), function() {
                    self.pathWay.run(null, function() {
                        if (self.mainLabel.value < (self.value + self.zjxwAppP)) {
                            if (d <= (self.value + self.zjxwAppP) - self.mainLabel.value) {
                                self.mainLabel.increase(d);
                            } else {
                                self.mainLabel.increase((self.value + self.zjxwAppP) - self.mainLabel.value);
                            }
                        }
                    });
                });
