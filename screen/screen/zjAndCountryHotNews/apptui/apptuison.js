...
App.prototype.requestData = function() {
    ...
    var url =  '/xhs/wcm/bigdata.do?loadtime='+getNowFormatDate()+'&modelid=newappcluefocus&pagesize=30&serviceid=cluefocus&startpage=0&typeid=widget'
...
App.prototype.renderList = function(data) {
    ...
    update.each(function(d, i) {
        ...
        // console.log(d);
        // console.log(d);
        var contentInner = d3.select(this).select('.content-inner').text(d.CONTENT.replace("null",""));
        d3.select(this).select('.REPORTNUM').text(d.PRCOUNT);
        d3.select(this).select('.EVENTTIPPING').text(d.RDCOUNT);
        d3.select(this).select('.FIRSTMEDIA').text(d.SITENAME);
        d3.select(this).select('.content').attr("sid",d.SID);
        d3.select(this).select('.FIRSTTIME').text(d.URLTIME);
    ...
    $("#list1 .content").off('click').on('click',function () {
        console.log($(this).find(".content-inner").html());
        var html = $(this).find(".content-inner").html()
        layer.open({
            title: "稿件详情",
