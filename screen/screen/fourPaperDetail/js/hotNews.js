...
(function() {
    ...
    Hotnews.prototype.getHotnews = function(success) {
        ...
        HS.httpServer("/isearch/front/search.jhtml?type="+GetQueryString('type')+"&code=551b1ff558194a0eb61d171ef40f3085&banci=A01&fastTime="+today+"&pageNumber=1&pageSize=10", params, "get").done(function(data) {
            appendList(obj, data, name);
            //释放内存，防止浏览器崩溃
            name = null;
            obj = null;
            params = null;
    ...
    function appendList(obj, data, name) {
        ...
        for (var i = 0; i < data.page.content.length; i++) {
            ...
            var _data = data.page.content[i];
            templateCopy.attr("sid",_data._id);
            // console.log(_data);
            ...
            templateCopy.find(".source-inner").html(_data.source);
            // templateCopy.find(".value").html(_data._id);
    ...
    function getDocDetail($liDom) {
        ...
        HS.httpServer("/isearch/front/search/detail.jhtml?code=551b1ff558194a0eb61d171ef40f3085", params, "get").done(function(data) {
            data = data.document;
            ...
            detailDom.find(".manuscript_resource").html(data.source);
            detailDom.find(".manuscript_date").html(data.date.substring(11));
