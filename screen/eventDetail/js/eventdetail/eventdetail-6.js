...
App.prototype.renderSection6 = function() {
    ...
    if($.isEmptyObject(self.sectionData_6) == false) {
       ...
       // console.log(feelingfallsData);
        ...
        self.currTime = fallsarray.length;
        ...
        /////////////////////////////
          ...
          //  console.log(feelingfallsData);
          //  console.log(endtime);
            var maxValueToday = d3.max(sectionData, function (d, i) {
                return Math.max(d.YESTERDAYVALUE, d.TODAYVALUE);
            });
            var maxValueChange = d3.max(sectionData, function (d, i) {
                return Math.max(Math.abs(d.CHANGE));
            });
            var maxValue;
            if (maxValueToday>maxValueChange) {
            	maxValue =  maxValueToday
            }else{
...
//			var maxValue = Math.max(maxChange,maxCout);
            ...
            update
                .each(function (d, i) {
                    ...
                    //	console.log(d.CHANGE)
                        rect0.attr({
                            'y': -self.yScale(d.CHANGE),
                            'height': self.yScale(d.CHANGE) - 100
                        });
                    }
                    else if (d.CHANGE <= 0) {
                        rect0.attr({
                           // 'y': -self.yScale(-2*d.CHANGE),
                             'y': -100,
                            'height': self.yScale(-d.CHANGE) - 100
                        });
                    }
        ...
        self.nexttoolindex = function () {
            ...
            self.tooltipindex++;
            if (self.tooltipindex >= self.currTime) {
                self.tooltipindex = 0;
            }
        };
        self.tooltipindex = 0;
        ...
        function renderxy(sectionData, endtime, maxValue) {
            //渲染X Y轴
            var starttime = endtime - self.currTime * 24 * 60 * 60 * 1000;
