...
App.prototype.requestData = function() {
	...
	var url = '/wcm/bigdata.do?modelid=hotpoint&serviceid=areahotpoint&loaddate='+today+'&area=001';
...
App.prototype.render = function(data) {
	...
	data.forEach(element => {
		this.total += parseInt(element.HOTPOINTNUM);
	});
	data.forEach(element => {
		element.percent =  parseInt(element.HOTPOINTNUM)/this.total*100;
	});
...
App.prototype.renderList = function(data) {
	...
	update.each(function(d, i) {
		...
		d3.select(this).select('.value').text(d.HOTPOINTNUM);
		d3.select(this).select('.color').style('opacity', opacityScale(d.percent));
		d3.select(this).select('.color').style('width', d.percent + '%');
...
App.prototype.scrolleOnFocusText = function(text) {
	window.setTimeout(function() {
		...
		if (containerWidth < textWidth) {
			$text.animate({
				left: textWidth - containerWidth + 800 + 'px'
			}, {
				step: function(now, fx) {
					$(this).css('transform', 'translate(-' + now + 'px,0px)');
				},
				duration: (textWidth / containerWidth) *6500,
				easing: 'linear',
				complete: function() {
