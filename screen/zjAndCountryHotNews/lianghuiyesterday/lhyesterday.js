...
App.prototype.requestData = function() {
    ...
    var url =  '/lhjk/getJson?name=eventOld'
...
App.prototype.renderList = function(data) {
    ...
    update.each(function(d, i) {
        ...
        d3.select(this).select('.REPORTNUM').text(d.REPORTNUM);
        d3.select(this).select('.EVENTTIPPING').text(d.EVENTTIPPING3.split(";")[0].split(":")[0]);
        d3.select(this).select('.content').attr("sid",d.CLUSTERID);
        d3.select(this).select('.FIRSTMEDIA').text(d.FIRSTMEDIA);
        d3.select(this).select('.FIRSTTIME').text(d.FIRSTTIME);
