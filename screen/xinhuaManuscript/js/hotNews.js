...
(function() {
    ...
    Hotnews.prototype.getHotnews = function(success) {
        ...
        HS.httpServer("/isearch/front/search.jhtml?code=551b1ff558194a0eb61d171ef40f3085&line=%E9%80%9A%E7%A8%BF&pageNumber=1&pageSize=10&searchWord=&type=30", params, "get").done(function(data) {
            console.log(3);
            appendList(obj, data.page.content, name);
            //释放内存，防止浏览器崩溃
            name = null;
            obj = null;
            params = null;
    ...
    function appendList(obj, data, name) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var _data = data[i];
            templateCopy[0].GUID = _data._id; //data[i].ID;
            ...
            console.log(_data.FIRSTMEDIA);
            templateCopy.find(".content-inner").html((i + 1) + "." + dom.innerText);
            templateCopy.find(".bar-inner").html(_data.source || "其他");
    ...
    function getDocDetail($liDom) {
        ...
        HS.httpServer("/isearch/front/search/detail.jhtml?code=551b1ff558194a0eb61d171ef40f3085", params, "get").done(function(data) {
            ...
            // detailDom.find(".manuscript_date").html(data['时间'].substring(11));
