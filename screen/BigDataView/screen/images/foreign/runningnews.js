 ...
 */
 ...
 function windowLink(e){
     var guid=e.split("?")[1];
     var data=bigDataUrl+XNbigUrl+'?modelid=detail&serviceid=hotsearch&'+guid;
     $.ajax({
        type:"get",
        url:wcmUrl,
        data:{
        ...
        success:function(htmls){
            console.log(htmls)
               ...
               if(htmls.DATA.length=='0'||htmls.DATA.ISSUCCESS=='false'){
                ...
                laytpl($NEWCONTENTTPL).render(htmls.DATA.PAGEITEMS[0].CONTENT.replace(/[\r\n]+/gi,"<br>"),function(html){
                    layer.open({
                        title:htmls.DATA.PAGEITEMS[0].TITLE,
    ...
    r.attr("class", "measure"),
      ...
      //   r.html(pr_titles[i].title),
        pr_titles[i].link!=null&&pr_titles[i].link.length>0?
            ...
            r.html("<span onclick=\"windowLink('"+pr_titles[i].link+"')\">"+pr_titles[i].title+"</span>")
            :r.html(pr_titles[i].title),
