...
$("body").delegate('.jsList',"click",function(){
    ...
    if(prams.length>0){
        for(var j in prams){
            tableList+="<a  target=\"value\" href=\""+prams[j].URLNAME+"\"><div class=\"item-list clear\" >"+
                "<div class=\"titleNum1\"></div>"+
                "<div class=\"title\" title=\""+prams[j].WEIBOCONTENT+"\">"+prams[j].WEIBOCONTENT+"</div>"+
                "</div></a>"
        }
...
App.prototype.requestJson = function(time) {
    var self = this;
    var datas="/lhjk/getJson?name=weibo";
    //var url=wcmUrl+"?url="+encodeURIComponent(datas);
    var url= datas;
    ...
    d3.json(url, function(error, data) {
        ...
        // microList.microList = data.DATA.DATA;
        // self.convertData(data.DATA.DATA);
...
App.prototype.getLists = function(id,keywords,hotname,eventId){
	...
	*/
    //d3.json(timeUrl,function(error,data){
    //	if (error || $.isEmptyObject(data) == true) {
    //		console.warn('error:');
    //		console.warn(error);
    //		return;
...
App.prototype.convertData = function(data){
    ...
    $(".microblog-hot-list").on('click',function(){
        ...
        for(var j in data){
            ...
            tableList+="<div class=\"item-list clear jsList\"  data-id=\""+data[j].id+"\" data-KEYWORDS=\""+data[j].keywords+"\" data-EVENTNAME=\""+data[j].eventname+"\" data-eventId=\""+data[j].event_id+"\">"+
                "<div class=\"titleNum1\"></div>"+
		...
		$(".micro-item-content-tabel-tbody .item-list").on('click',function(){
			...
			self.getLists1(tid,tKEYWORDS,tEVENTNAME,teventId,true);
