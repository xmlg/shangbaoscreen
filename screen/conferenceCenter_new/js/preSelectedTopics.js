...
angular.module("preSelectedTopicsModule", ["ngAnimate", "mgcrea.ngStrap", "httpServiceModule", "dataManagerModule", "scrollBarModule", "switchDraftModule", "videoPlayerModule"])
    .controller("preSelectedTopicsCtrl", ["$scope", "$q", "$sce", "$timeout", "dataManagerService", function($scope, $q, $sce, $timeout, dataManagerService) {
        ...
        $scope.queryOper = function(item) {
            item.oper = "下载";
            if (item.REPORTFILEEXT.toLowerCase() === "mp4" || item.REPORTFILEEXT.toLowerCase() === "mp3" || item.REPORTFILEEXT.toLowerCase() === "image" || item.REPORTFILEEXT.toLowerCase() == "jpg" || item.REPORTFILEEXT.toLowerCase() == "png" || item.REPORTFILEEXT.toLowerCase() == "gif" || item.REPORTFILEEXT.toLowerCase() == "bmp" || item.REPORTFILEEXT.toLowerCase() == "jpeg") {
                item.oper = "查看";
            }
        };
        /**
