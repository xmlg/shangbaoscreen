define(function(require) {
	...
	function SubSceneEdit(parent) {
		...
		this.mainLabel = new MainLabel(this, '编', data.TOTAL);
		...
		this.auditInfoPanel = new AuditInfoPanel(this, [
			[data.WAITAUDIT.ONE, data.WAITAUDIT.TWO, data.WAITAUDIT.THREE, data.WAITAUDIT.FOUR, data.WAITAUDIT.FIVE],
			[data.AUDITED.ONE, data.AUDITED.TWO, data.AUDITED.THREE, data.AUDITED.FOUR, data.AUDITED.FIVE]
		]);
	...
	SubSceneEdit.prototype.runPathWay = function(data, newData, ms) {
		...
		for(var i = 0; i < names.length; i++) {
			...
			var incrementA = newData.AUDITED[name] - data.AUDITED[name];
			...
			if(incrementA > 0) {
				...
				self.auditInfoPanel.audit(5, mySelf.value);
			}
