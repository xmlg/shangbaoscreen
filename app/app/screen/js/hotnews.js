...
var MTdata={serviceid:'hotpointevent1', typeid:"widget",modelid:'hotpointnews',
    field:"",loadtime:"",user_id:"admin",department:140,pagesize:20,startpage:0};
...
function renderList(data,pn,sortName,sortType){
    ...
    for(var i=0;i<listLength;i++){
        // console.log(data.PAGEITEMS[i])
        content += '<li class="news_list_center_li">' +
                        '<span class="news_list_center_num">'+((pNum)*10+i+1)+'</span>'+
                        '<ul class="news_list_center_ul">'+
                        "<li><a target='_blank' href='/dist/index.html#/plan/hotpointcluster?guids="+data.PAGEITEMS[i].CLUSTERID+"'>" +data.PAGEITEMS[i].TITLE+ (data.PAGEITEMS[i].ISTIPPINGPOINT==1?logoImg:'')+ (data.PAGEITEMS[i].STATUS==0?newImg:'') + "</a></li>"+
...
function setContentHtml(item){
    ...
    hotNum.forEach(function(data,index,array){
        ...
        data.forEach(function(_data,_index,_array){
            ...
            str+="<a target='_blank' href='/dist/index.html#/plan/hotpointcluster?guids="+_data.CLUSTERID+"'>"+_data.SHORTTITLE+"</a>";//CUSTER_NUMS
