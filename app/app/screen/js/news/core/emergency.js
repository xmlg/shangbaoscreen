zbdp.loadEmergency = function() {
    ...
    //var emergencyUrl = 'js/news/core/tufashijian.json';
    var xhsHotUrl = '/wcm/bigdata.do?modelid=wechatDetail&user_id=admin&department=140518&serviceid=wechatranklist1&typeid=widget';
    //var xhsHotUrl = 'js/news/core/hotRank.json';
    //var nationUrl = 'js/news/core/hotRank.json';
    var nationUrl = '/wcm/bigdata.do?modelid=wechatDetail&user_id=admin&department=140518&serviceid=wechatranklist1&typeid=widget&catalogid=0';
    var hotRank = {
        "1":[],
        "2":[],
        "3":[],
        ...
        $.ajax({
            ...
            success: function(datastr){
                ...
                $.each(data,function(i,n){
                    ...
                    //console.log(n[0])
                    item.ID = n[0].ir_hkey;
                    item.SID = n[0].ir_weixinid;
                    item.DATASOURCE = "微信榜单";
                    item.SITENAME = n[0].ir_authors;
                    item.URLTITLE = n[0].ir_urltitle;
                    //str=str.replace(/> \s+ </g, "> < ");
                    var regx=/<[^>]*>|<\/\/[^>]*>/gm;
                    item.URLABSTRACT = n[0].ir_content.replace(/\r\n/gmi,'').replace(regx,"").trim().substring(0,266)+'...';
                    item.Rate = label;
                    item.STATUS = "1";
                    item.TIME = n[0].ir_urltime;
                    item.LOADTIME = n[0].ir_urltime;
                    item.MARK = "0";
                    hotRank[label].push(item);
                })
                if(label == 2){
                    isShow(hotRank)
                }
